...
class Debugger
{
    ...
    /** Whether to use the PSR-7 dispatcher */
    public static bool $usePsr7 = false;
     ...
     *
     * @param  bool|string|string[]  $mode  use constant Debugger::Production, Development, Detect (autodetection) or
     *   IP address(es) whitelist.
    ...
    public static function exceptionHandler(\Throwable $exception) : void
    {
        ...
        if (!headers_sent()) {
            http_response_code(isset($_SERVER['HTTP_USER_AGENT']) && str_contains($_SERVER['HTTP_USER_AGENT'],
              'MSIE ') ? 503 : 500);
        }
        ...
        if (empty(self::$strategy[self::$productionMode])) {
            self::$strategy[self::$productionMode] = self::$productionMode
              ...
              : new DevelopmentStrategy(
                ...
                self::getBlueScreen(),
                self::$usePsr7
                  ? new DeferredContentPsr7(self::getSessionStorage())
    ...
    public static function dispatchPsr7(ServerRequestInterface $request) : ResponseInterface
    {
        return self::getStrategy()->sendAssetsPsr7($request);
    }
    ...
    public static function shutdownHandler() : void
    {
        ...
        if (in_array($error['type'] ?? null,
          [E_ERROR, E_CORE_ERROR, E_COMPILE_ERROR, E_PARSE, E_RECOVERABLE_ERROR, E_USER_ERROR], true)) {
            self::exceptionHandler(new ErrorException($error['message'], 0, $error['type'], $error['file'],
              $error['line']));
    ...
    public static function barDump(mixed $var, ?string $title = null, array $options = []) : mixed
    {
        if (!self::$productionMode) {
            ...
            $panel->data[] = [
              'title' => $title, 'dump' => Dumper::toHtml($var, $options + [
                  Dumper::DEPTH    => self::$maxDepth,
                  Dumper::ITEMS    => self::$maxItems,
                  Dumper::TRUNCATE => self::$maxLength,
